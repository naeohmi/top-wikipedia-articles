...
const WIKIMEDIA_API_BASE_URL = "https://wikimedia.org/api/rest_v1/metrics/pageviews/top-per-country";
...
const GridDashboard = () => {
  ...
  const [countrySearchValue, setCountrySearchValue] = useState(DEFAULT_COUNTRY_VALUE);
  const [isLoading, setIsLoading] = useState(true);
  const [isError, setIsError] = useState(false);
  const [wikiArticles, setWikiArticles] = useState(undefined);

  useEffect(() => {
    const dateUrlParms = `${dateValue.format("YYYY")}/${dateValue.format("MM")}/${dateValue.format("DD")}`;
    const wikiApiDateAndCountryUrlParams = `${countrySearchValue}/all-access/${dateUrlParms}`;

    const wikiArticleFetch = async (baseApiUrl, urlParams) => {
      try {
        const res = await fetch(`${baseApiUrl}/${urlParams}`);
        const apiResponse = await res.json();
        const articlesByResultLimit = apiResponse.items[0].articles.slice(0, resultsLimit);
    ...
    wikiArticleFetch(WIKIMEDIA_API_BASE_URL, wikiApiDateAndCountryUrlParams);
